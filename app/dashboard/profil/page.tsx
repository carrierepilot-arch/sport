...
export default function ProfilPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        // Mettre à jour les données locales avec les valeurs renvoyées par le serveur
        setPseudo(data.user.pseudo ?? '');
        setName(data.user.name ?? '');
    ...
    <main className="flex-1 px-4 py-6 sm:px-8 sm:py-10">
      <div className="max-w-4xl">
        <div className="mb-8">
          ...
          <p className="text-gray-500 mt-1">Gerez vos informations personnelles et vos preferences.</p>
        </div>
